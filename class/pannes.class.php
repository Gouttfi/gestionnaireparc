...
/**
 * \file        class/pannes.class.php
 * \ingroup     gestionnaireparc
 * \brief       This file is a CRUD class file for Pannes (Create/Read/Update/Delete)
 */
 ...
 * Class for Pannes
 */
class Pannes extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'pannes';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'gestionnaireparc_pannes';
	...
	/**
	 * @var string String with name of icon for pannes. Must be the part after the 'object_' into object_pannes.png
	 */
	public $picto = 'pannes@gestionnaireparc';
	...
	public $fields=array(
		...
		'fk_date_intervention' => array('type'=>'datetime', 'label'=>'DateReparation', 'enabled'=>'1', 'position'=>9, 'notnull'=>-1, 'visible'=>5, 'noteditable'=>'1',),
	...
	//  */
	// public $table_element_line = 'gestionnaireparc_pannesline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_pannes';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Pannesline';
	...
	//  */
	// protected $childtablesoncascade = array('gestionnaireparc_pannesdet');

	// /**
	//  * @var PannesLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->gestionnaireparc->pannes->read) {
	...
	public function create(User $user, $notrigger = false)
	{
		...
		// Changement de l'état de la machine concernée par l'état "en panne"
		$sql = "UPDATE `".MAIN_DB_PREFIX."gestionnaireparc_machines` SET `etat_actuel` = '1', `stat_nb_pannes` = `stat_nb_pannes`+1
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->gestionnaireparc->pannes->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->gestionnaireparc->pannes->pannes_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('PANNES_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'pannes/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'pannes/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->gestionnaireparc->dir_output.'/pannes/'.$oldref;
				$dirdest = $conf->gestionnaireparc->dir_output.'/pannes/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gestionnaireparc->dir_output.'/pannes/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'PANNES_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'PANNES_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'PANNES_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$hookmanager->initHooks(array('pannesdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new PannesLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_pannes = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->GESTIONNAIREPARC_PANNES_ADDON)) {
			$conf->global->GESTIONNAIREPARC_PANNES_ADDON = 'mod_pannes_standard';
		}

		if (!empty($conf->global->GESTIONNAIREPARC_PANNES_ADDON)) {
			$mybool = false;

			$file = $conf->global->GESTIONNAIREPARC_PANNES_ADDON.".php";
			$classname = $conf->global->GESTIONNAIREPARC_PANNES_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_pannes';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->PANNES_ADDON_PDF)) {
				$modele = $conf->global->PANNES_ADDON_PDF;
			}
		}
...
/**
 * Class PannesLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PannesLine extends CommonObjectLine
{
	// To complete with content of an object PannesLine
	// We should have a field rowid, fk_pannes and position
