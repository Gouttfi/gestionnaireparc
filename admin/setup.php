...
/**
 * \file    gestionnaireparc/admin/setup.php
 * \ingroup gestionnaireparc
 * \brief   GestionnaireParc setup page.
 */
...
require_once '../lib/gestionnaireparc.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "gestionnaireparc@gestionnaireparc"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('gestionnaireparcsetup', 'globalsetup'));
...
$arrayofparameters = array(
	...
	//'GESTIONNAIREPARC_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'GESTIONNAIREPARC_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'GESTIONNAIREPARC_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf GESTIONNAIREPARC_MYPARAM3
	$item = $formSetup->newItem('GESTIONNAIREPARC_MYPARAM3');
	$item->setAsThirdpartyType();

	// Setup conf GESTIONNAIREPARC_MYPARAM4 : exemple of quick define write style
	$formSetup->newItem('GESTIONNAIREPARC_MYPARAM4')->setAsYesNo();

	// Setup conf GESTIONNAIREPARC_MYPARAM5
	$formSetup->newItem('GESTIONNAIREPARC_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf GESTIONNAIREPARC_MYPARAM6
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/gestionnaireparc/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'GESTIONNAIREPARC_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'GESTIONNAIREPARC_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$page_name = "GestionnaireParcSetup";
...
$head = gestionnaireparcAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "gestionnaireparc@gestionnaireparc");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("GestionnaireParcSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'GESTIONNAIREPARC_'.strtoupper($myTmpObjectKey).'_ADDON';
