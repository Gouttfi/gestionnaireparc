...
/**
 *   	\file       interventions_card.php
 *		\ingroup    gestionnaireparc
 *		\brief      Page to create/edit/view interventions
 */
...
dol_include_once('/gestionnaireparc/lib/gestionnaireparc_interventions.lib.php');
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'interventionscard'; // To manage different context of search
...
$hookmanager->initHooks(array('interventionscard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$upload_dir = $conf->gestionnaireparc->multidir_output[isset($object->entity) ? $object->entity : 1].'/interventions';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/gestionnaireparc/interventions_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/gestionnaireparc/interventions_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'GESTIONNAIREPARC_INTERVENTIONS_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'GESTIONNAIREPARC_INTERVENTIONS_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_INTERVENTIONS_TO';
	$trackid = 'interventions'.$object->id;
...
if ($action == 'create') {
	...
	print '<table class="conditionnal border centpercent tableforfieldcreate">'."\n";
...
if (($id || $ref) && $action == 'edit' || $action == 'edit_avant_realiser') {
	...
	print '<table class="conditionnal border centpercent tableforfieldedit">'."\n";
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create' && $action != 'edit_avant_realiser'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Interventions"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteInterventions'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/gestionnaireparc/interventions_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('gestionnaireparc:Interventions', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('interventions'));
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', dol_buildpath('/gestionnaireparc/interventions_agenda.php', 1).'?id='.$object->id);
...
// End of page
print '<script>';
include DOL_DOCUMENT_ROOT.'/custom/gestionnaireparc/js/interventions.js';
print '</script>';
llxFooter();
$db->close();
